...
func TestBindingNewBindinger(t *testing.T) {
	assert.Equal(t, Form, NewBindinger("GET", ""))
	assert.Equal(t, Form, NewBindinger("GET", MIMEJSON))

	assert.Equal(t, JSON, NewBindinger("POST", MIMEJSON))
	assert.Equal(t, JSON, NewBindinger("PUT", MIMEJSON))

	assert.Equal(t, XML, NewBindinger("POST", MIMEXML))
	assert.Equal(t, XML, NewBindinger("PUT", MIMEXML2))

	assert.Equal(t, Form, NewBindinger("POST", MIMEPOSTForm))
	assert.Equal(t, Form, NewBindinger("PUT", MIMEPOSTForm))

	assert.Equal(t, FormMultipart, NewBindinger("POST", MIMEMultipartPOSTForm))
	assert.Equal(t, FormMultipart, NewBindinger("PUT", MIMEMultipartPOSTForm))
}
...
func testBodyBinding(t *testing.T, b Bindinger, name, path, badPath, body, badBody string) {
...
func testBodyBindingSlice(t *testing.T, b Bindinger, name, path, badPath, body, badBody string) {
	assert.Equal(t, name, b.Name())
...
func testBodyBindingStringMap(t *testing.T, b Bindinger, path, badPath, body, badBody string) {
	obj := make(map[string]string)
	req := requestWithBody("POST", path, body)
	if b.Name() == "form" {
		req.Header.Add("Content-Type", MIMEPOSTForm)
...
func testBodyBindingUseNumber(t *testing.T, b Bindinger, name, path, badPath, body, badBody string) {
	assert.Equal(t, name, b.Name())
	...
	v, e := obj.Foo.(ojson.Number).Int64()
	assert.NoError(t, e)
...
func testBodyBindingUseNumber2(t *testing.T, b Bindinger, name, path, badPath, body, badBody string) {
	assert.Equal(t, name, b.Name())
...
func testBodyBindingDisallowUnknownFields(t *testing.T, b Bindinger, path, badPath, body, badBody string) {
	originJson := json
...
func testBodyBindingFail(t *testing.T, b Bindinger, name, path, badPath, body, badBody string) {
	assert.Equal(t, name, b.Name())
